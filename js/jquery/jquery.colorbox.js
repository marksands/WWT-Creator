...
(function($){
	...
	*/
	...
	$.fn.colorbox.dimensions = function(object){
		...
		$loaded.fadeIn(100); // added to remove the preloader
		...
		if (settings.transition == 'fade') {
			...
			$modal.fadeTo(speed, 100, function(){setPosition(0);});
		} else {
			setPosition(speed);
		}
	...
	$.fn.colorbox.load = function(){
		...
		} else { //if (isImage(href)){ // Mark's LAME HACK
	...
	/*	} else {
			$('<div />').load(href, function(data, textStatus){
				if(textStatus == "success"){
					...
					$.fn.colorbox.dimensions($(this)); // Mark's lame hack

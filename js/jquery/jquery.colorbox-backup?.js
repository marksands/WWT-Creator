	...
	$.fn.colorbox = function(options, custom_callback) {
		...
		$(this).unbind("click.colorbox").bind("click.colorbox", function (event) {
			...
			if (!open) {
				...
				open = true;
				$.fn.colorbox.position(setSize(settings.initialWidth, 'x'), setSize(settings.initialHeight, 'y'), 0).show();
	...
	*/
	$.fn.colorbox.init = function(){
		...
		$modalContent.append(
			...
			// Inventive hack to hide the preloader :)

			$loaded = $('<div id="cboxLoadedContent" style="width:0; height:0;" />'),
			$loadingOverlay = $('<div id="cboxLoadingOverlay-break" />'),
			$loadingGraphic = $('<div id="cboxLoadingGraphic-break" />'),
	...
	$.fn.colorbox.dimensions = function(object){
		...
		//$loaded.appendTo('body')  This sounds like I'm on the right track here <<<<<<<<
		...
		$loaded.fadeIn(100);
		//$loaded.fadeTo(speed, 0, function(){setPosition(0);}); // this is the fix :), shows the box at full 100
		//$loaded.fadeIn(100); // this at least shows the content in the preloader... i need to stop the fadeout :\
		//$loaded.fadeTo(speed, 100, function() {setPosition(0);});
		...
		if (settings.transition == 'fade') {
			$modal.fadeTo(speed, 100, function(){setPosition(0);}); //here's the kicker, skips the fade (0-100), and stays at 100
